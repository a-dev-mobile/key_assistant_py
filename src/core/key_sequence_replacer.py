...
class KeySequenceReplacer:
    def __init__(self, yaml_path, delay, device_path):
        logging.debug(f'Initializing KeySequenceReplacer with yaml_path: {yaml_path}, delay: {delay}, device_path: {device_path}')
    ...
    def update_clipboard_buffers(self):
        ...
        if new_clipboard != self.current_clipboard:
            logging.debug(f'Clipboard changed from {self.current_clipboard} to {new_clipboard}')
    ...
    def insert_text_from_buffer(self, position, length):
        try:
            ...
            logging.info(f"Inserting text from buffer position {position}: {text_to_insert}")
            ...
            if self.first_match:
                logging.info(f"Inserting text again due to first_match: {text_to_insert}")
            ...
            error_message = f"Error inserting text from buffer: {e}\n{traceback.format_exc()}"
            ...
            if isinstance(sequence['keys'][0], list):  # Handle common replacements
                for key_group in sequence['keys']:
                    if self.key_sequence[-len(key_group):] == key_group:
                        ...
                        if sequence['action'] == 'replace':
                            ...
                            self.insert_text_from_buffer(sequence['position'], len(key_group))
                        return
            else:
                if self.key_sequence[-len(sequence['keys']):] == sequence['keys']:
                    ...
                    if sequence['action'] == 'replace':
                        ...
                        self.insert_text_from_buffer(sequence['position'], len(sequence['keys']))
                    return

    def on_key_event(self, key_name, key_code, key_type, key_state):
        logging.debug(f'Key event - Name: {key_name}, Code: {key_code}, Type: {key_type}, State: {key_state}')
